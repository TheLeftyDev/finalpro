...
import {fullSlider } from '@/app/Mockdata';
const FullSlider = () => {
    const leftref = useRef<HTMLButtonElement>(null)
    const rightref = useRef<HTMLButtonElement>(null)
    const [i, seti] = useState(0)
    const nextSlide = () => {
      if (i === fullSlider.length - 1) {
        //do nothing
    ...
    if(i === fullSlider.length - 1 && rightref.current){rightref.current.style.opacity = '0.3';}
    else if(i !== fullSlider.length - 1 && rightref.current){rightref.current.style.opacity = '1';}
